...
export class InitialSchema1637985998271 implements MigrationInterface {
    name = 'InitialSchema1637985998271'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`applications\` (\`applicantId\` int UNSIGNED NOT NULL, \`announcementId\` int UNSIGNED NOT NULL, \`state\` enum ('P', 'A', 'D') NOT NULL DEFAULT 'P', \`id\` bigint UNSIGNED NOT NULL AUTO_INCREMENT, \`date\` date NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`payments\` (\`charge_amount\` int NOT NULL, \`company\` varchar(70) NOT NULL, \`Payment_Option\` varchar(20) NOT NULL, \`suscriptions\` varchar(150) NOT NULL, \`payment_date\` varchar(500) NOT NULL, \`id\` bigint UNSIGNED NOT NULL AUTO_INCREMENT, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
    }
